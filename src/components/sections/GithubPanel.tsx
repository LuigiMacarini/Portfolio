...
export async function GithubPanel({ dict }: { dict: Dictionary["github"] }) {
  ...
  return (
    <Section id="github">
      ...
      {!stats ? (
        <Reveal delay={120}>
          <p className="mt-10 max-w-md font-mono text-xs uppercase tracking-wide text-mist-500">
            {dict.unavailable}
          </p>
        </Reveal>
      ) : (
        <div className="mt-14 grid grid-cols-1 gap-10 lg:grid-cols-12">
          <Reveal delay={100} className="lg:col-span-4">
            <dl className="grid grid-cols-3 divide-x divide-line border border-line lg:grid-cols-1 lg:divide-x-0 lg:divide-y">
              ...
              <div className="mt-6 border border-line p-5">
                ...
                <div className="mt-4 flex flex-col gap-3">
                  {stats.languages.map((lang) => (
                    <div key={lang.name}>
                      <div className="flex items-center justify-between font-mono text-[11px] uppercase tracking-wide text-mist-600">
                        <span>{lang.name}</span>
                        <span>{lang.percent}%</span>
                      </div>
                      <div className="mt-1.5 h-1 w-full bg-mist-100">
            ...
            <div className="mt-4 grid grid-cols-1 gap-px border border-line bg-line sm:grid-cols-2">
              {stats.featured.map((repo) => (
                <a
                  ...
                  className="group flex flex-col gap-2 bg-paper p-5 transition-colors hover:bg-mist-50"
                >
                  ...
                  {repo.description && (
                    <p className="text-xs leading-relaxed text-mist-500">{repo.description}</p>
                  )}
                </a>
              ))}
            </div>
      ...
      <Reveal delay={200}>
        <p className="mt-10 font-mono text-[10px] uppercase tracking-wider text-mist-400">
          {dict.note}
        </p>
      </Reveal>
    </Section>
